...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE hardwareproduct SET nama=%s, deskripsi=%s, harga=%s, foto=%s WHERE id=%s",
					   ...
					   move_uploaded_file($_FILES['foto']['tmp_name'],"producthardware/".$_FILES['foto']['name']);

  mysql_select_db($database_config, $config);
  $Result1 = mysql_query($updateSQL, $config) or die(mysql_error());

  $updateGoTo = "productlisthardware.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
mysql_select_db($database_config, $config);
$query_producthardware = sprintf("SELECT * FROM hardwareproduct WHERE id = %s", GetSQLValueString($colname_producthardware, "int"));
    ...
    <div class="container">
        ...
        <div class="box">
            <div class="navigation">
                <p><a href="dashboard.php">Dashboard</a> > <a href="categorylist.php">Category List</a> > <a href="productlisthardware.php">Hardware Product</a> > Edit Product</p>
            </div>
            <div class="name-tag">
                <h2>Edit Product</h2>
            </div>
            <div class="box-menu">
                <form action="<?php echo $editFormAction; ?>" method="post" enctype="multipart/form-data" name="form1" id="form1">
                    <div class="user-box">
                        ...
                        <input type="text" name="nama" value="<?php echo htmlentities($row_producthardware['nama'], ENT_COMPAT, 'UTF-8'); ?>" size="32" />
                    </div>
                    <div class="user-box">
                        <label for="nama">Harga Product</label>
                        <input type="text" name="harga" value="<?php echo htmlentities($row_producthardware['harga'], ENT_COMPAT, 'UTF-8'); ?>" size="32" />
                    </div>
                    <div class="user-box">
                        <label for="username">Gambar Product</label>
                        <input class="foto" type="file" name="foto" value="<?php echo htmlentities($row_producthardware['foto'], ENT_COMPAT, 'UTF-8'); ?>" size="32" />
                    </div>
                    <div class="user-box1">
                      <label>Deskripsi Produk</label>
                      <textarea class="sejarah" name="deskripsi"><?php echo htmlentities($row_producthardware['deskripsi'], ENT_COMPAT, 'UTF-8'); ?></textarea>
                    </div>
                    <div class="button">
                      <input class="tbl-login" type="submit" name="register" value="Edit Product" />
                    </div>
                    <input type="hidden" name="MM_update" value="form1" />
      			    <input type="hidden" name="id" value="<?php echo $row_producthardware['id']; ?>" />
                </form>
            </div>
        </div>
    </div>
